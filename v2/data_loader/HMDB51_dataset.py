...
class HMDB51(TextVideoDataset):
    def _load_metadata(self):
        # download specific
        metadata_dir = 'meta_data'
        split_files = {
            'train': 'hmdb51/prompt/split_1_train_list.tsv',
            'val': 'hmdb51/prompt/split_1_test_list.tsv',  # there is no test
            'test': 'hmdb51/prompt/split_1_test_list.tsv',
        }
    ...
    def _get_video_path(self, sample):
        return os.path.join(self.data_dir, sample[0]), sample[0]
    ...
    def __getitem__(self, item):
        ...
        data = {'video': final, 'text': 'NULL', 'label': sample[1] - 1, 'meta': meta_arr, 'keep_ind': keep_ind}
        return data
